...
workflow PROTEIN_ANNOTATION {
    ...
    if (params.protein_annotation_tool == 'InterProScan') {
        ...
        if (params.protein_annotation_interproscan_db != null) {
            ch_interproscan_db = channel.fromPath(params.protein_annotation_interproscan_db)
                .first()
        }
        else {
        ...
        ch_interproscan_tsv_modified = INTERPROSCAN.out.tsv.map { meta, tsv_path ->
            def modified_tsv_path = "${workflow.workDir}/tmp/${meta.id}_interproscan.faa.tsv"

            def modified_tsv_content = new File(tsv_path.toString()).readLines().collect { line -> (line.split('\t') + ['NA', 'NA']).join('\t') }
